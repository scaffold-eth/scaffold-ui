...
| `debounceDelay` | `number` _(optional)_ | `500`   | Debounce delay in milliseconds for ENS resolution.        |
...
:::info
`ensAddress`, `ensName`, and `ensAvatar` fields are tri-state: `undefined` (not requested, still resolving, or error), `null` (no record), or a value (resolved).
:::
