...
| `isError`                      | `boolean`                        | Error state for the balance fetch only. A price-fetch failure is exposed separately via `isNativeCurrencyPriceError` so it doesn't blank the balance UI. |
