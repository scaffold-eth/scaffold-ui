...
When an invalid Ethereum address is provided, the component displays an error message with "Invalid address" text. The error color uses the `--color-sui-error` CSS variable, which can be customized in your theme.
...
To customize the error color, override the CSS variable in your theme:

```css
:root {
  --color-sui-error: #ff8863;
}
```
