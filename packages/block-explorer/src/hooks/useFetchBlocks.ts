...
import { decodeTransactionData, GenericContractsDeclaration } from "../utils/decodeTxData";
...
export const useFetchBlocks = (deployedContracts: GenericContractsDeclaration | null) => {
  ...
  const fetchBlocks = useCallback(async () => {
    ...
    try {
      ...
      fetchedBlocks.forEach((block) => {
        block.transactions.forEach((tx) => decodeTransactionData(tx as Transaction, deployedContracts));
      });
      ...
      try {
        if (currentPage === 0) {
          ...
          newBlock.transactions.forEach((tx: Transaction) =>
            decodeTransactionData(tx as Transaction, deployedContracts),
          );
