...
const ENUM_STATUSES = {
  ...
  loading: <span className="w-4 loading-dc loading-dc-spinner"></span>,
  error: <ExclamationCircleIcon className="w-7 text-error" />,
  info: <InformationCircleIcon className="w-7 text-sui-primary" />,
...
const Notification = ({
  ...
  return toast.custom(
    (t: Toast) => (
      <div
        className={`flex flex-row items-start justify-between max-w-sm rounded-xl shadow-center shadow-sui-primary bg-sui-primary-neutral p-4 transform-gpu relative transition-all duration-500 ease-in-out space-x-2
        ${
